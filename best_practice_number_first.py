...
from utils import число, строка, рандом_рандинт, список, написать
...
def функция_которая_возвращает_когото(аргумент_номер_один:dict|list,аргумент_номер_два:list|dict,тут_цифру_передай:int)->list:
 ...
 лист_номер_три_с_новыми_значениями:list=список()
 список_три_с_половиной_на_всякий_случай:list=список()
 цифра = число(0)
...
def функция_номер_два(ничего_здесь_не_передавай:None=None):
 return ничего_здесь_не_передавай if ничего_здесь_не_передавай!=None else список().append([в_этот_раз_не_хэ_а_б for в_этот_раз_не_хэ_а_б in строка(строка((рандом_рандинт(1,10)+рандом_рандинт(1,10)*рандом_рандинт(1,10)))+строка('боже')+строка(",")+""+строка("что")+""+строка("я")+""+строка("делаю?"))])
